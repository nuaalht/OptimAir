...
%     title(['Estimated time ' num2str(propagatedState.totalTime) 's'])
    ...
    uicontrol('Parent',ita_fig,...
              'Units','pixels',...
              'Position',[640 335 55 25],...
              'BackgroundColor',[1 0.65 0],...
              'ForegroundColor','w',...
              'String','Auto',...
              'Callback',@ITA_auto_Callback);
    ...
    function ITA_auto_Callback(~,~)
        ...
        for ii = 1:numel(initVels)
            for jj = 1:numel(windDirs)
                for kk = 1:numel(windVels)
                    configuration.dynamics.initVel = initVels(ii);
                    configuration.dynamics.windHeading = windDirs(jj);
                    configuration.dynamics.windVel = windVels(kk);
                    % Recalculate wind components
                    configuration.dynamics.windVelocityEarth = calculateWindComponents(configuration);
                    ITA_finish_Callback;
                    copyfile('OutputFiles/*',['AutoGenerated/' initVelString{ii}...
                        '/' windDirString{jj} '/' windVelString{kk}]);
                    cab(f,ita_fig); % Close all but GUI and ITA
                end
            end
            configuration.dynamics.initVel = initVels(ii);
            configuration.dynamics.windHeading = 0;
            configuration.dynamics.windVel = 0;
            % Recalculate wind components
            configuration.dynamics.windVelocityEarth = calculateWindComponents(configuration);
            ITA_finish_Callback;
            copyfile('OutputFiles/*',['AutoGenerated/' initVelString{ii}...
